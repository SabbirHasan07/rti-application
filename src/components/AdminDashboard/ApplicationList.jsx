...
export const ApplicationList = ({ allApplications, setSelected, currentPage, itemsPerPage }) => {
    ...
    console.log({allApplications, paginatedData})
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto">
                {paginatedData.map((item, index) => (
                    <div key={index} className="border p-3 rounded-lg flex justify-between">
                        <div>
                            <p><strong>অ্যাপ্লিকেশন আইডি: </strong> {item.data.application_id}</p>
                            <p><strong>নাম:</strong> {item.data.name}</p>
                            <p><strong>তারিখ ও সময়: </strong> {new Date(item.data.createdAt).toLocaleString()}</p>
                        </div>
                        <button
